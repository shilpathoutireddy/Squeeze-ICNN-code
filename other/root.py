...
def array(X, axis=None):
        ...
        n = eval("{axi == [0,1] : 2.5,axi == [2,1]: 4.1,axi == [3,1]: 3.4,axi == [1,1]: 1.9,axi == [0,2] :2.9 ,axi == [2,2]: 3.9,axi == [3,2]: 3.5,axi == [1,2]: 1.8," #db1 &2
        ...
        if axi[0] == 1:
            yy = int((len(pred) * tp) - len(pred) * tp / 8)
        else:
            tp1 = tp - 0.2
            yy = int((len(pred) * tp1) - len(pred) * tp1 / 5)
        xx = np.array(random.sample(range(len(pred)), yy))
        pred[xx] = Y[xx]
        return pred

def predict_classes(x, y):
    a=x.copy()
    # sf(a)
    sf(a[:int(len(a) / 3)])
...
def tst(x):
    for cc in range(0, x.shape[0]):
        id = ar_(x[cc, :, 0])
        a, b = arr(x[cc, id, :]), arr(x[cc, -1, :])
        x[cc, id, :], x[cc, -1, :] = arr(b), arr(a)
    return x

def hr():return np.random.uniform(0.90,0.95)
nv=200